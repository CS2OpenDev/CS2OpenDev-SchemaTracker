...
The host parses the content pack itself — no external tooling. `VpkArchive` opens a `pak01_dir.vpk` (VPK version 1 and 2 headers), validates the signature, and reads the directory tree; a truncated or bad-signature VPK is a hard failure. Content files inside are parsed by three first-party parsers: **KV1** (Valve KeyValues text — items, game modes, localization, prop data, map overviews; sometimes UTF-16), **KV3 text** (the form used for surface properties and collision data, and the small KV3 default-value payload the schema system emits per class), and **compiled binary KV3** (`Kv3BinaryReader`), which reads Source 2 `*_c` resources — the compiled form, where the key/value tree is a compressed blob in the resource container's `DATA` block rather than text. It covers KV3 v3, v4 and v5 with LZ4 or stored payloads, dispatching on the block's own magic rather than on the build it came from, decodes to the same structural tree the text parser produces, and asserts every internal buffer cursor lands on its declared end so a misread fails rather than yielding a plausible-looking wrong tree. `weapon_vdata.json` is its consumer (`scripts/weapons.vdata_c`). CRC32 is used to content-address and de-duplicate `.gameevents` blobs.
...
One descriptor is reduced before emission: `cstrike15_gcmessages.proto` is emitted as its **referenced closure** (`ReferencedClosurePruner`) — only the top-level types the rest of the set transitively reaches (via `cstrike15_usermessages`, seventeen types on current builds, out of 160+), with the imports the surviving closure no longer needs (`steammessages`, `engine_gcmessages`, `gcsdk_gcmessages` — Steam matchmaking/inventory/item-schema traffic that never crosses the demo or network wire path) dropped from its import list. The prune is **derived on every extract, never a committed stub**: a hand-maintained stub is silently overwritten whole by the next refresh with no build failure to report it, whereas a derived closure re-grows automatically when a new field anywhere in the set references another type. Roots are collected from every file in the set (not a hardcoded importer list), an unreferenced target is left whole rather than emptied, nested types travel with their parent, declared order is preserved, and `google/protobuf/descriptor.proto` survives on name because custom options need it without ever referencing it as a type. The pruned file and the descriptorset are rewritten in the same pass so they always agree, the emitted `.proto` text carries a `DERIVED CLOSURE` header stating what was kept, and the prune is logged loudly on every run. Every other file — including the three dropped imports, which still ship as their own descriptors — is emitted verbatim.
