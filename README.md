...
|---|---|---|
| `entity_schema.json` | The core artifact: every schema class/struct and enum, with fields (offset, size, type, metadata), parent chains, and KV3 default values. Type names are the contract for the handle family (`CHandle< T >`, `CStrongHandle< T >`, …) — spec in [docs/SCHEMA_HANDLES.md](docs/SCHEMA_HANDLES.md). | The Source 2 **schema system**, walked live in-process. |
...
| Artifact | What it is | Sourced from |
|---|---|---|
| `gameevents.json` | The game-event registry (event names and their fields), structurally parsed. Merges the csgo pak's `game.gameevents` + `mod.gameevents` with the engine core pak's `core.gameevents` (the engine event registry) when that pak is present. Field **types carry wire semantics**: when the engine builds the demo/network event descriptor (`CMsgSource1LegacyGameEventList`), a field of type `player_controller_and_pawn` expands into **two** wire keys — `<name>` (controller slot) plus `<name>_pawn` (pawn ehandle) — and a `player_pawn` field emits `<name>_pawn`; `player_controller` emits `<name>` alone. The declarations here are verbatim; generators deriving per-event key sets must apply that expansion (measured: it reproduces the observed descriptor key set — see issue #6). | `.gameevents` files inside `game/csgo/pak01_dir.vpk` and `game/core/pak01_dir.vpk`. |
